...
visualize_graphs <- function(metadata,rels, graph_name){
  ...
  df_rel <- data.frame(df_rel$node1, df_rel$node2)
  g <- graph.data.frame(df_rel)
  # add node sizing - indegree
  df.g <- data.frame(video = V(g)$name,
                     indegree_norm = degree(g, mode = "in", normalized = F))
  V(g)$indegree=df.g$indegree_norm[match(V(g)$name,df.g$video)] # This code 
...
get_groups <- function(graph, df_meta, clique_n){
...
get_correlation_matrix <- function(df.g, cf, norm, title){
  ...
  if (cf == FALSE){ # if network is inappropriate, you have to turn that col into a numeric value 
    ...
    if (norm == TRUE){
      M <-cor(df.g[, c(2,3,4,8,9,10,12,13)])
    }
    else{
      M <-cor(df.g[, c(5,6,7,8,9,10,12,13)])
    }
  }
  else{
    # get correlations
    if (norm == TRUE){
      M <-cor(df.g[, c(2,3,4,8,9,10,11)])
    }
    else{
      M <-cor(df.g[, c(5,6,7,8,9,10,11)])
    }
  }
...
data_bad_measures[, c(2,3,4,8,9,10,11,12)]
get_correlation_matrix(data_bad_measures, cf=FALSE, norm=TRUE, 'Inappropriate Network')
get_correlation_matrix(data_bad_measures, cf=FALSE, norm=FALSE, 'Inappropriate Network')
...
get_correlation_matrix(data_good_measures, cf=TRUE, norm=FALSE, 'Child-Friendly Network')
